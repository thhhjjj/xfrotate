...
// 升级文件放到外挂flash里面用于测试双备份升级
    ...
    device = device_open(EXT_FLASH_RW);
    err = vfs_mount(&pvfs, device, "sydfile");
    vfs_demo_show("vfs_mount\n", err);
    err = vfs_openbypath(pvfs, &pvfile, "/db_data.bin");    
    vfs_demo_show("vfs_openbypath\n", err);
    
    u32 f_offset=0,len_cnt=0;
    struct vfs_attr file_attr;
    u8 *fr_buf = (u8 *)my_malloc(512, 0);
    while (1) {
        u32 rlen = 0;
        vfs_seek(pvfile, f_offset, SEEK_SET);
        rlen = vfs_read(pvfile, fr_buf, 512);
        len_cnt += rlen;
        if (rlen == 0) {
            break;
        }
    ...
    u32 data_crc = 0;
    while (file_size) {
        vfs_seek(pvfile, offset, 0);
        u32 cnt = file_size > 512 ? 512 : file_size;
        vfs_read(pvfile, tmp_buf, cnt);
...
void get_dual_bank_info(void){
    u32 res;
    u32 upgrade_start_addr = 0;
    u32 bank_size;
    res = jlfs_get_idle_bank_info(&upgrade_start_addr, &bank_size);
    r_printf("addr %x,size %x\n", upgrade_start_addr, bank_size);
    }